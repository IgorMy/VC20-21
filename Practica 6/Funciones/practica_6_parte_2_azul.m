function practica_6_parte_2_azul(Nombre)
...
I = imread("Imagenes/SegundaParte/"+Nombre);
figure,imshow(I),title("Imagen RGB");pause;
...
figure,imshow(Ib),title("Detección");pause;
...
plot(p2,p1,'*r')
hold off;
title("Visualización de la detección sobre la imagen original");pause;
...
%      ¿Cuántos puntos de Ib contiene esta circunferencia?
    ...
    plot(c+cp,f+fp,'.r');
    hold off
    title("Detección de forma circular");pause;
...
figure,imshow(Ib_circunf),title("Detección y binarización de forma circular"),pause;
