...
%% Consultar tipo de variable, generar y cargar archivos .mat
...
clear
load('datos/variables','suma','resta')  
...
% Eliminar de B la el número de la segunda columna: 
B = B(1,[1 3:end])
...
% Ejercicio: crea 1000 valores aleatorios que sigan una distribución normal
% de media 0 y desviación típica 1. Repetir para una media 20 y desviación 10
x = randn(1000,1); hist(x)
media = 20; desv = 10; x = media + desv*randn(1000,1); figure, hist(x)
...
% SE DEBE UTILIZAR EL FORMATO TIPO CELL (MATRICES DE CELDAS, CADA CELDA
...
addpath('../temp') % Retrocede un directorio y busca el directorio temp
load variables.mat
...
%% OPERADORES RELACIONALES, LÓGICOS Y SENTENCIAS DE CONTROL DE FLUJO
% eq - Equal = =
% ne - Not equal ~ =
% lt - Less than <
% gt - Greater than >
...
% Ejercicio1: 
% a) Crea un vector V de 10 elementos con números aleatorios entre 0 y 10.
...
% posiciones donde se encuentran en V
        ...
        [ordenados, posicionesVector] = sort(V);
...
% aparece en A cada uno de sus posiles valores
    ...
    for i=1:size(valoresA,2)
        conteoValores(i) = sum(sum(A==valoresA(i)));
    end
